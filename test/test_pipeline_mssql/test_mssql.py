...
def queries_results():
    results = execute_data_queries_from_postgresql(
        queries={'contract_prices':'select count(*) from dwh."I_ContractPrices" cp where cp."ProjectId"=\'S18\' and cp."ContractPrice" is not null;',
                 'volume_hedge':'select count(*) from dwh."I_Hedge" h where h."ProjectId"=\'S18\' and h."P50H" is not null;',
                 'sta_production':'select round(sum(pa."P50")/1000, 2) as production from stagging."ProductionAsset" as pa where pa."Year"=2022 and pa."ProjectId" in (select a."ProjectId" from stagging."Asset" a where a."InPlanif"=False);',
                 'sta_productibles':'select round(sum(a."P50")/1000,2) as productibles from stagging."Asset" a where a."InPlanif" = false;',
                 'dwh_production':'select round(sum(ia."P50A")/1000, 2) as production from dwh."I_Asset" ia where ia."Year"=2022 and ia."ProjectId" in (select da."ProjectId" from dwh."D_Asset" da where da."InPlanif"=False);',
